...
static class ReleaseUpdaterTests {
    ...
    sealed class Fixture {
        ...
        internal LocalFakeGitHubTransport Transport() {
            var routes = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) {
                { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(ReleaseJson) } },
                { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13", new ResponseSpec { Body = Encoding.UTF8.GetBytes(ManifestJson) } },
                { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/14", new ResponseSpec { Body = PatchJar } }
    ...
    static Dictionary<string, ResponseSpec> LatestOnly(string json, int status) { return new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { StatusCode = status, Body = Encoding.UTF8.GetBytes(json) } } }; }
    static Dictionary<string, ResponseSpec> LatestOnlySpec(ResponseSpec spec) { return new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", spec } }; }
    ...
    static void TestHttpFailureMatrix(string root) {
        ...
        var routes = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(fixture.ReleaseJson) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13", new ResponseSpec { Body = Encoding.UTF8.GetBytes(fixture.ManifestJson), ContentLength = Encoding.UTF8.GetByteCount(fixture.ManifestJson) - 1 } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/14", new ResponseSpec { Body = fixture.PatchJar } } }; using (var server = ServerFor(routes)) { var release = WakfuReleaseUpdater.GetLatestRelease(server); ExpectFailure("manifest Content-Length", delegate { WakfuReleaseUpdater.GetManifest(release, server); }); }
        var manifest404 = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(fixture.ReleaseJson) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13", new ResponseSpec { StatusCode = 404, Body = Encoding.UTF8.GetBytes("missing") } } }; using (var server = ServerFor(manifest404)) { var release = WakfuReleaseUpdater.GetLatestRelease(server); ExpectFailure("manifest download 404", delegate { WakfuReleaseUpdater.GetManifest(release, server); }); }
        var i18n404 = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(fixture.ReleaseJson) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13", new ResponseSpec { Body = Encoding.UTF8.GetBytes(fixture.ManifestJson) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/14", new ResponseSpec { StatusCode = 404, Body = Encoding.UTF8.GetBytes("missing") } } }; using (var server = ServerFor(i18n404)) { var release = WakfuReleaseUpdater.GetLatestRelease(server); var manifest = WakfuReleaseUpdater.GetManifest(release, server); ExpectFailure("i18n download 404", delegate { WakfuReleaseUpdater.DownloadPatch(release, manifest, Path.Combine(root, "missing.jar"), server); }); }
        byte[] manifestBytes = Encoding.UTF8.GetBytes(fixture.ManifestJson); routes["/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13"] = new ResponseSpec { Body = manifestBytes.SubArray(0, manifestBytes.Length - 2) }; using (var server = ServerFor(routes)) { var release = WakfuReleaseUpdater.GetLatestRelease(server); ExpectFailure("truncated manifest", delegate { WakfuReleaseUpdater.GetManifest(release, server); }); }
        var patchShort = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(fixture.ReleaseJson) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13", new ResponseSpec { Body = manifestBytes } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/14", new ResponseSpec { Body = fixture.PatchJar.SubArray(0, fixture.PatchJar.Length - 1) } } }; using (var server = ServerFor(patchShort)) { var release = WakfuReleaseUpdater.GetLatestRelease(server); var manifest = WakfuReleaseUpdater.GetManifest(release, server); ExpectFailure("truncated i18n", delegate { WakfuReleaseUpdater.DownloadPatch(release, manifest, Path.Combine(root, "short.jar"), server); }); }
        var contentRoutes = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(fixture.ReleaseJson) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13", new ResponseSpec { Body = manifestBytes } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/14", new ResponseSpec { Body = fixture.PatchJar, ContentLength = fixture.PatchJar.Length + 1 } } }; using (var server = ServerFor(contentRoutes)) { var release = WakfuReleaseUpdater.GetLatestRelease(server); var manifest = WakfuReleaseUpdater.GetManifest(release, server); ExpectFailure("i18n Content-Length", delegate { WakfuReleaseUpdater.DownloadPatch(release, manifest, Path.Combine(root, "content-mismatch.jar"), server); }); }
        string wrongManifest = ManifestJsonFor(new String('0', 64), fixture.PatchJar.Length, fixture.SourceHash); var wrongHashRoutes = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(ReleaseJsonFor(Encoding.UTF8.GetByteCount(wrongManifest), fixture.PatchJar.Length)) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13", new ResponseSpec { Body = Encoding.UTF8.GetBytes(wrongManifest) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/14", new ResponseSpec { Body = fixture.PatchJar } } }; using (var server = ServerFor(wrongHashRoutes)) { var release = WakfuReleaseUpdater.GetLatestRelease(server); var manifest = WakfuReleaseUpdater.GetManifest(release, server); ExpectFailure("SHA mismatch", delegate { WakfuReleaseUpdater.DownloadPatch(release, manifest, Path.Combine(root, "sha.jar"), server); }); }
        string mismatchManifest = ManifestJsonFor(fixture.PatchHash, fixture.PatchJar.Length + 1, fixture.SourceHash); var mismatchRoutes = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(ReleaseJsonFor(Encoding.UTF8.GetByteCount(mismatchManifest), fixture.PatchJar.Length)) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13", new ResponseSpec { Body = Encoding.UTF8.GetBytes(mismatchManifest) } } }; using (var server = ServerFor(mismatchRoutes)) { var release = WakfuReleaseUpdater.GetLatestRelease(server); ExpectFailure("manifest size mismatch", delegate { WakfuReleaseUpdater.GetManifest(release, server); }); }
        var dropRoutes = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(fixture.ReleaseJson) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13", new ResponseSpec { Body = manifestBytes } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/14", new ResponseSpec { Body = fixture.PatchJar, ContentLength = fixture.PatchJar.Length, DropConnection = true } } }; using (var server = ServerFor(dropRoutes)) { var release = WakfuReleaseUpdater.GetLatestRelease(server); var manifest = WakfuReleaseUpdater.GetManifest(release, server); ExpectFailure("connection drop", delegate { WakfuReleaseUpdater.DownloadPatch(release, manifest, Path.Combine(root, "drop.jar"), server); }); }
        var redirect = new ResponseSpec { StatusCode = 302, Location = "http://api.github.com/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest" }; using (var server = ServerFor(LatestOnlySpec(redirect))) ExpectFailure("HTTPS downgrade redirect", delegate { WakfuReleaseUpdater.GetLatestRelease(server); }); var loop = new ResponseSpec { StatusCode = 302, Location = "https://api.github.com/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest" }; using (var server = ServerFor(LatestOnlySpec(loop))) ExpectFailure("redirect loop", delegate { WakfuReleaseUpdater.GetLatestRelease(server); }); var forbidden = new ResponseSpec { StatusCode = 302, Location = "https://evil.example/releases/latest" }; using (var server = ServerFor(LatestOnlySpec(forbidden))) ExpectFailure("non-allowlisted redirect", delegate { WakfuReleaseUpdater.GetLatestRelease(server); }); Require(!WakfuReleaseUpdater.IsAllowedDownloadUri(new Uri("http://api.github.com/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest")), "HTTPS downgrade allowed"); Require(!WakfuReleaseUpdater.IsAllowedDownloadUri(new Uri("https://evil.example/releases/latest")), "non-allowlisted redirect allowed");
    }
    ...
    static void TestInstallAndRollback(string root) {
        ...
        try {
            string wrongManifest = ManifestJsonFor(new String('0', 64), fixture.PatchJar.Length, fixture.SourceHash); var badRoutes = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) { { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(ReleaseJsonFor(Encoding.UTF8.GetByteCount(wrongManifest), fixture.PatchJar.Length)) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/13", new ResponseSpec { Body = Encoding.UTF8.GetBytes(wrongManifest) } }, { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/14", new ResponseSpec { Body = fixture.PatchJar } } }; bool preinstallFailed = false; using (var server = ServerFor(badRoutes)) { var release = WakfuReleaseUpdater.GetLatestRelease(server); var manifest = WakfuReleaseUpdater.GetManifest(release, server); try { typeof(WakfuSetupApp).GetMethod("TestInstallReleasedPatch", BindingFlags.Static | BindingFlags.NonPublic).Invoke(null, new object[] { game, release, manifest, server }); } catch (TargetInvocationException) { preinstallFailed = true; } } Require(preinstallFailed, "pre-install hash error did not fail"); Require(WakfuReleaseUpdater.Sha256File(source) == fixture.SourceHash, "pre-install error touched i18n_en"); Require(WakfuReleaseUpdater.Sha256File(old) == fixture.SourceHash, "pre-install error touched i18n");
    ...
    static void TestExternalFullSimulation(string fixtureRoot) {
        ...
        var routes = new Dictionary<string, ResponseSpec>(StringComparer.Ordinal) {
            { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/latest", new ResponseSpec { Body = Encoding.UTF8.GetBytes(releaseJson) } },
            { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/101", new ResponseSpec { Body = Encoding.UTF8.GetBytes(manifestText) } },
            { "/repos/Relactive/wakfu-turkiye-yama-calismasi/releases/assets/102", new ResponseSpec { Body = patch } }
        };
