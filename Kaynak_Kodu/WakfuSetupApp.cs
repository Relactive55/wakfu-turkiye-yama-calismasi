...
static class WakfuSetupApp {
    ...
    static Dictionary<string,string> PrepareOfficialSources(string game,string officialDir){
    ...
    static void InstallReleasedPatch(string game,LatestPatchRelease release,PatchManifest manifest,Action<string> progress){InstallReleasedPatchCore(game,release,manifest,progress,null);}
#if WAKFU_TESTS
    internal static void TestInstallReleasedPatch(string game,LatestPatchRelease release,PatchManifest manifest,IReleaseHttpTransport transport){WakfuReleaseUpdater.TestTransportOverride=transport;try{InstallReleasedPatchCore(game,release,manifest,null,transport);}finally{WakfuReleaseUpdater.TestTransportOverride=null;}}
#endif
    static void InstallReleasedPatchCore(string game,LatestPatchRelease release,PatchManifest manifest,Action<string> progress,IReleaseHttpTransport transport){
        if(!IsWakfu(game))throw new ReleaseUpdateException("Geçerli Wakfu klasörü seçilmedi.");if(release==null||manifest==null)throw new ReleaseUpdateException("İndirilecek Release bilgisi yok.");EnsureClosed(game);WakfuReleaseUpdater.ValidateReleaseContract(release,manifest);VerifyReleaseBaseline(game,manifest);
        ...
        try{string downloaded=Path.Combine(transaction,manifest.File);if(progress!=null)progress("İndiriliyor...");WakfuReleaseUpdater.DownloadPatch(release,manifest,downloaded);if(progress!=null)progress("Doğrulanıyor...");WakfuReleaseUpdater.VerifyDownloadedPatch(downloaded,manifest);VerifyJar(downloaded,"texts_en.properties");VerifyJar(downloaded,"texts_en_cleaned.properties");live=GameFiles(game);backup=BackupReleaseTargets(live,manifest);var staged=new Dictionary<string,string>(StringComparer.OrdinalIgnoreCase);staged[live["i18n_en"]]=downloaded;staged[live["i18n"]]=downloaded;if(progress!=null)progress("Kuruluyor...");CommitReleaseI18nTransaction(staged,transaction);filesCommitted=true;if(!String.Equals(HashFile(live["i18n_en"]),manifest.Sha256,StringComparison.OrdinalIgnoreCase)||!String.Equals(HashFile(live["i18n"]),manifest.Sha256,StringComparison.OrdinalIgnoreCase))throw new ReleaseUpdateException("Kurulum sonrası i18n dosyası doğrulanamadı.");WriteReleasedPatchState(game,release,manifest,backup);UpdateLegacyInstallStateI18n(game,manifest.Sha256);}
    ...
    static void Install(string game,string overheadProfile){
        overheadProfile=NormalizeOverheadProfile(overheadProfile);if(!IsWakfu(game))throw new Exception("Geçerli Wakfu klasörü seçilmedi.");EnsureClosed(game);VerifyEmbeddedPackage();if(IsFullyCurrentInstallation(game,overheadProfile)){try{WriteOverheadPreference(overheadProfile);}catch{}return;}string transaction=Path.Combine(Path.GetTempPath(),"WakfuTurkceKurulum_"+Guid.NewGuid().ToString("N"));Directory.CreateDirectory(transaction);
        try{
            var live=GameFiles(game);var official=PrepareOfficialSources(game,Path.Combine(transaction,"official"));string patchedDir=Path.Combine(transaction,"patched");Directory.CreateDirectory(patchedDir);var staged=new Dictionary<string,string>(StringComparer.OrdinalIgnoreCase);
            string prepared=BuildAdaptivePatch(official["i18n_en"]);string patchedEn=Path.Combine(patchedDir,"i18n_en.jar"),patchedActive=Path.Combine(patchedDir,"i18n.jar");File.Move(prepared,patchedEn);File.Copy(patchedEn,patchedActive,true);staged[live["i18n_en"]]=patchedEn;staged[live["i18n"]]=patchedActive;
